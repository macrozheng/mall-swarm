...
CREATE TABLE `oms_order` (
  ...
  `status` int(1) DEFAULT NULL COMMENT 'Order status: 0->to be paid; 1->to be shipped; 2->shipped; 3->completed; 4->closed; 5->invalid order',
...
CREATE TABLE `oms_order_operate_history` (
  ...
  `order_status` int(1) DEFAULT NULL COMMENT 'Order status: 0->to be paid; 1->to be shipped; 2->shipped; 3->completed; 4->closed; 5->invalid order',
...
CREATE TABLE `pms_product_attribute` (
  ...
  `product_attribute_category_id` bigint(20) DEFAULT NULL,
  `name` varchar(64) DEFAULT NULL,
  `select_type` int(1) DEFAULT NULL COMMENT 'Attribute selection type: 0->only; 1->single selection; 2->multiple selection',
  `input_type` int(1) DEFAULT NULL COMMENT 'Attribute entry method: 0->manual entry; 1->select from the list',
  `input_list` varchar(255) DEFAULT NULL COMMENT '可选值列表，以逗号隔开',
  `sort` int(11) DEFAULT NULL COMMENT '排序字段：最高的可以单独上传图片',
  `filter_type` int(1) DEFAULT NULL COMMENT '分类筛选样式：1->普通；1->颜色',
  `search_type` int(1) DEFAULT NULL COMMENT 'Search type; 0->no need to search; 1->keyword search; 2->range search',
  `related_status` int(1) DEFAULT NULL COMMENT 'Whether products with the same attribute are related; 0->not related; 1->related',
  `hand_add_status` int(1) DEFAULT NULL COMMENT 'Whether to support manual addition; 0->not supported; 1->support',
